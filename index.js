...
const allowedOrigins = [
  ...
  "https://life-insurance-server-three.vercel.app"
];
...
function buildPolicyFromBody(body, isUpdate = false) {
  /* Keep your existing buildPolicyFromBody implementation exactly as is */
  const {
    title,
    category,
    policyType,
...
let usersCollection, policiesCollection, applicationsCollection, 
    reviewsCollection, blogsCollection, paymentsCollection, 
    claimsCollection, newsletterCollection;
...
async function run() {
  try {
    ...
    await claimsCollection.createIndex({ email: 1 });
    await claimsCollection.createIndex({ applicationId: 1 });
    ...
    app.patch("/agent/applications/:id/status", async (req, res) => {
      try {
        ...
        if (!isValidObjectId(id)) return res.status(400).json({ message: "Invalid application id" });
        if (!["Pending", "Approved", "Rejected"].includes(status)) {
          return res.status(400).json({ message: "Invalid status" });
        }
        ...
        console.error("Update status (agent):", err);
        res.status(500).json({ message: "Internal Server Error" });
      }
    });
    ...
    app.get('/payments/summary', async (req, res) => {
      try {
        const summary = await paymentsCollection.aggregate([
          {
            $group: {
              _id: null,
              totalUSD: { $sum: "$amountUSD" },
